...
class MergedMiningStratumProtocol(JSONRPC):
    ...
    def handle_authorize(self, params: Union[List, Dict], msgid: Optional[str]) -> None:
        ...
        if self.coordinator.address_from_login:
            try:
                ...
                self.payback_script_hathor = create_output_script_htr(decode_address(payback_address_hathor))
                self.payback_script_bitcoin = create_output_script_btc(decode_address(payback_address_bitcoin))
...
class BitcoinCoordJob(NamedTuple):
    ...
    def make_coinbase_transaction(self, hathor_block_hash: bytes, payback_script_bitcoin: bytes, bitcoin_reward: int,
        ...
        coinbase_output = BitcoinTransactionOutput(bitcoin_reward, payback_script_bitcoin)
...
class MergedJob(NamedTuple):
    ...
    def new_single_miner_job(self, protocol: MergedMiningStratumProtocol) -> SingleMinerJob:
        ...
        coinbase_tx = self.bitcoin_coord.make_coinbase_transaction(
            ...
            self.bitcoin_coord.coinbase_value,
            xnonce_size,
        )
...
class MergedMiningCoordinator(Factory):
    ...
    def __init__(self, port: int, bitcoin_rpc: IBitcoinRPC, hathor_client: IHathorClient, payback_address_bitcoin: str,
                 payback_address_hathor: str, address_from_login: bool = True, reactor: IReactorTCP = reactor):
        self.log = logger.new()
        self.port = port
